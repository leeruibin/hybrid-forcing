...
from utils.wan_wrapper import WanHybridAttnWrapper


class HybridForcingInferencePipeline(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
        ...
        model_kwargs["casual_wan_type"] = "HybridCausalWanModelInfer"
        self.generator = WanHybridAttnWrapper(
            **model_kwargs, image_or_video_shape=args.image_or_video_shape, is_causal=True) if generator is None else generator
        self.text_encoder = WanTextEncoder(args.model_kwargs.get("pretrained_model_name_or_path", "wan_models/Wan2.1-T2V-1.3B")) if text_encoder is None else text_encoder
        self.vae = WanVAEWrapper(args.model_kwargs.get("pretrained_model_name_or_path", "wan_models/Wan2.1-T2V-1.3B")) if vae is None else vae
        ...
        self.frame_seq_length = self.image_or_video_shape[-1] * self.image_or_video_shape[-2] // math.prod(self.generator.model.patch_size[-2:])
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        kv_cache_size = self.frame_seq_length * 24 # WARNING: Hard coded
        head_dim = self.generator.model.config.dim // self.generator.model.config.num_heads
        for _ in range(self.num_transformer_blocks):
            kv_cache_clean.append({
                "k": torch.zeros([batch_size, kv_cache_size, self.generator.model.config.num_heads, head_dim], dtype=dtype, device=device),
                "v": torch.zeros([batch_size, kv_cache_size, self.generator.model.config.num_heads, head_dim], dtype=dtype, device=device),
                "global_end_index": 0,
                "local_end_index": 0,
            })
