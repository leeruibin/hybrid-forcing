...
class CausalWanSelfAttention(nn.Module):
    ...
    def update_global_with_kv(self, k, v, grid_sizes, freqs, global_kmean, global_seqlen, global_kv, current_start_frame=0):
        b, s, n, d = k.shape
        ...
        k_rope = k_rope.permute(0, 2, 1, 3)  # B, seq, num_heads, head_dim -> B, h, seq, h_d

        k = k.permute(0, 2, 1, 3)  # B, seq, num_heads, head_dim -> B, h, seq, h_d
        v = v.permute(0, 2, 1, 3)  # B, seq, num_heads, head_dim -> B, h, seq, h_d
        updated_kmean = (k.mean(dim=-2, keepdim=True).transpose(-2, -1) * s + global_kmean * global_seqlen) / (s + global_seqlen) 
        updated_kv = ((k_rope.transpose(-2, -1)) @ v + global_kv * global_seqlen ) / (global_seqlen + s)
        update_seqlen = s + global_seqlen
        
        return updated_kmean, updated_kv, update_seqlen
        
    def global_linear(self, q, grid_sizes, freqs, global_kmean, global_seqlen, global_kv, current_start_frame=0):
        if global_seqlen.item() == 0:
            return 0

        q = self.kernel_func(q)  # B, h, h_d, N

        q_rope = causal_rope_apply(
                    q, grid_sizes, freqs, start_frame=current_start_frame)

        q = q.permute(0, 2, 1, 3)  # B, seq, num_heads, head_dim -> B, h, seq, h_d
        q_rope = q_rope.permute(0, 2, 1, 3)  # B, seq, num_heads, head_dim -> B, h, seq, h_d
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            evicted_tokens = current_end > kv_cache["global_end_index"].item()

            if evicted_tokens and (num_new_tokens + kv_cache["local_end_index"].item() > kv_cache_size):
            ...
            updated_globel_linear_state = global_linear_state
            if is_first_step:
                # TODO hard coding now, 查找 Px chunk前的第一个chunk 用于更新global linear state
                ...
                if linear_chunk_start_idx >= 0:
                    ...
                    updated_kmean, updated_kv, update_seqlen = self.update_global_with_kv(linear_k,linear_v, grid_sizes,freqs,global_kmean, global_seqlen, global_kv, current_start_frame=linear_start_rela_findex)

                    updated_globel_linear_state["global_linear_kmean"] = updated_kmean
                    updated_globel_linear_state["global_linear_seqlen"] = update_seqlen
                    updated_globel_linear_state["global_linear_kv"] = updated_kv
            ...
            if ancher_frame_len > 0 and self.rope_type == "relative":
                ancher_k = causal_rope_apply(
                        ancher_k, grid_sizes_relative, freqs, start_frame=0).type_as(v)
            ...
            if previous_frame_len > 0 and self.rope_type == "relative":
            ...
            global_kmean = updated_globel_linear_state["global_linear_kmean"]
            global_seqlen = updated_globel_linear_state["global_linear_seqlen"]
            global_kv = updated_globel_linear_state["global_linear_kv"]


            x_linear = self.global_linear(q, grid_sizes,freqs,global_kmean, global_seqlen, global_kv,current_start_frame=current_q_start_frame)
        ...
        x = x.flatten(2)
        x = self.o(x)
        return x, updated_globel_linear_state
...
class HybridCausalWanModel(ModelMixin, ConfigMixin):
    r"""
