    ...
    def forward(self, x, context, context_lens, crossattn_cache=None):
        ...
        if crossattn_cache is not None:
            if not crossattn_cache["is_init"]:
                ...
                k_full = self.norm_k(self.k(context)).view(b, -1, n, d)
                v_full = self.v(context).view(b, -1, n, d)
                crossattn_cache["k"] = k_full
                crossattn_cache["v"] = v_full
            k = crossattn_cache["k"]
            v = crossattn_cache["v"]
        else:
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C]  (or [B, L1_local, C] if SP)
            context(Tensor): Shape [B, L2, C]
            context_lens(Tensor): Shape [B] (for the *text* part only)
        """
        # Split out image context and text context
        context_img = context[:, :257]     # [B, L_img_ctx, C]
        context_txt = context[:, 257:]    # [B, L_txt, C]

        b = x.size(0)
        n = self.num_heads
        d = self.head_dim

        q = self.norm_q(self.q(x)).view(b, -1, n, d)   # [B, L, H, D]

        k_img_full = self.norm_k_img(self.k_img(context_img)).view(b, -1, n, d)  # [B, L_img_ctx, H, D]
        v_img_full = self.v_img(context_img).view(b, -1, n, d)                   # [B, L_img_ctx, H, D]

        k_txt_full = self.norm_k(self.k(context_txt)).view(b, -1, n, d)          # [B, L_txt, H, D]
        v_txt_full = self.v(context_txt).view(b, -1, n, d)                       # [B, L_txt, H, D]


        k_img = k_img_full                                # [B, L_img_ctx, H, D]
        v_img = v_img_full
        k_txt = k_txt_full                                # [B, L_txt, H, D]
        v_txt = v_txt_full

        img_x = flash_attention(q, k_img, v_img, k_lens=None)                # [B, L1, H_local or H, D]
        x_txt = flash_attention(q, k_txt, v_txt, k_lens=context_lens)        # [B, L1, H_local or H, D]

        img_x = img_x.flatten(2)   # [B, L1_local or L1, H*D]
        x_txt = x_txt.flatten(2)   # [B, L1_local or L1, H*D]

        x = x_txt + img_x          # [B, L1_local or L1, H*D]
        x = self.o(x)              # [B, L1_local or L1, C]
