...
def compress_kernel_bsld(
    ...
    """(B, L, H, D) 布局: seq 在 dim=1，stride 为 H*D."""
    idx_l = tl.program_id(0)
    idx_bh = tl.program_id(1)
    ...
    offs_d = tl.arange(0, D)
    stride_L = H * D
    x_base = (idx_bh // H) * L * H * D + (idx_bh % H) * D
    x = tl.load(
        X + x_base + offs_l[:, None] * stride_L + offs_d[None, :],
        mask=offs_l[:, None] < L,
    )
    nx = min(BLOCK_L, L - idx_l * BLOCK_L)
    x_mean = tl.sum(x, axis=0, dtype=tl.float32) / nx
...
def mean_pool_bsld(x, BLK):
    """x: (B, L, H, D)，输出 (B, H, L_BLOCKS, D)，不 transpose."""
    assert x.is_contiguous()
    B, L, H, D = x.shape
    L_BLOCKS = (L + BLK - 1) // BLK
    x_mean = torch.empty((B, H, L_BLOCKS, D), device=x.device, dtype=x.dtype)
    grid = (L_BLOCKS, B * H)
    compress_kernel_bsld[grid](x, x_mean, L, D, BLK, H)
    return x_mean


def get_block_map(q, k, topk_ratio, BLKQ=64, BLKK=64, layout_bhld=True):
    """
    layout_bhld: True 表示 q,k 为 (B, H, L, D)；False 表示 (B, L, H, D)。
    """
    ...
    if layout_bhld:
        ...
        pooled_qblocks = mean_pool_bsld(q, BLKQ)
        pooled_kblocks = mean_pool_bsld(arg_k, BLKK)
